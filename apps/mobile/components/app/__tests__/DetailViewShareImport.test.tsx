/* ============================================================
   ...
   the system share text (keeping the PNG card).

   Pins (these reference behavior NOT yet implemented → FAIL now, PASS once
   S3a lands):
   1. Pressing the importable-share entry calls mintShare(tasteId) and then
      shares text that contains BOTH the deepLink and the importCode.
...
describe('DetailView importable share (S3a)', () => {
  ...
  it('mints a token and PRINTS the import code on the ShareCard (the delivered channel)', async () => {
    ...
    const btn = findImportableShareBtn(r)
