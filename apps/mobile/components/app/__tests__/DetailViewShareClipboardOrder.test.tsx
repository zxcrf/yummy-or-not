...
async function pickImportable(r: TestRenderer.ReactTestRenderer) {
  ...
  // commits the import code / QR, then waits the readiness signal + a couple of
  // rAF "paint" frames before capturing, so the chain (mintShare → setState
  // commit → onReady → rAF → captureRef → shareAsync → marker → clipboard)
  // needs several interleaved timer + microtask passes under fake timers.
